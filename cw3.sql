...
TundraTrees
AS(
	SELECT tr.vegdesc AS tree, ST_Area(ST_Collect(ST_Intersection(tu.geom, tr.geom))) AS area
	FROM tundra tu, trees tr
	GROUP BY tr.vegdesc
),
SwampTrees
AS(
	SELECT tr.vegdesc AS tree, ST_Area(ST_Collect(ST_Intersection(s.geom, tr.geom))) AS area
